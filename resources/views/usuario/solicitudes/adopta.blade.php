...
<body id="page-top">
    ...
    <header class="masthead bg-primary text-white text-center">
        <div class="container">
            <h2 class="page-section-heading text-center text-uppercase text-secondary mb-0">Solicitudes</h2>
                            ...
                            @foreach ($solicitudes as $solicitud)
                                <div class="col-md-4 mb-4">
                                    <div class="card h-100">
                                        <img src="{{ $solicitud->mascota ? asset('http://localhost:8001/storage/' . $solicitud->mascota->foto1) : '' }}"
                                            class="card-img-top"
                                            alt="{{ $solicitud->mascota ? $solicitud->mascota->nombre : 'Imagen no disponible' }}">
                                        <div class="card-body">
                                            <h5 class="card-title">
                                                {{ $solicitud->user ? $solicitud->user->name : 'Usuario no encontrado' }}
                                                solicita a
                                                {{ $solicitud->mascota ? $solicitud->mascota->nombre : 'Mascota no encontrada' }}
                                            </h5>
                                            <p class="card-text"><strong>Fecha de solicitud:</strong>
                                                {{ $solicitud->created_at->format('d/m/Y H:i') }}</p>
                                            <p class="card-text"><strong>Estado de la solicitud:</strong>
                                                {{ $solicitud->estado }}</p>
                                            <!-- Aquí se muestra el estado de la solicitud -->
                                        </div>
                                    </div>
                                </div>
                            @endforeach
